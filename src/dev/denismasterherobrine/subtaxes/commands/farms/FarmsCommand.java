    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if (command.getName().equalsIgnoreCase("farms")) {
            if (commandSender instanceof Player) {
                if (strings.length == 0) {
                    commandSender.sendMessage("Command usage: /farms add/remove <name of the farm> to add/remove your own farm.");
                    commandSender.sendMessage("Command usage: /farms add/remove <name of the farm> decline to decline your request to add/remove your own farm.");
            ...
            if (strings.length == 1) {
                if (strings[0].equals("add")) {
                    if (commandSender instanceof Player) {
                        ...
                        int executingDayOfWeek = GetDate.getDayOfWeek();
                        if (executingDayOfWeek != 7) {
                            player.sendMessage("Укажите название фермы! Например: /farms add ферма_железа123");
                            return true;
                        } else player.sendMessage("Данная команда работает во все дни, кроме воскресенья!");
                    }
                }
                    ...
                    if (strings[1] != null) {
                        if (strings[2].equals("decline")) {
                            ...
                            if (executingDayOfWeek != 7) {
                                BukkitRunnable r = new BukkitRunnable() {
                                    ...
                                    public void run() {
                                        try {
                                            openConnection();
                                            Statement statement = connection.createStatement();
                                            String SQLString = "DELETE FROM FarmsTableTest WHERE type='0' AND PlayerUUID='" + player.getUniqueId() + "' AND name='" + strings[1] + "';";
                                            statement.executeUpdate(SQLString);
                                            connection.close();
                                        } catch (ClassNotFoundException | SQLException e) {
                                            e.printStackTrace();
                                ...
                                r.runTaskAsynchronously(JavaPlugin.getPlugin(SubTaxes.class));
                                player.sendMessage("Ваша ферма " + strings[1] + " успешно удалена из списка на рассмотрение!");
                                return true;
                            } else player.sendMessage("Данная команда работает во все дни, кроме воскресенья!");
                        }
                    }
                    ...
                    if (strings[1] != null) {
                        if (strings[2].equals("decline")) {
                            ...
                            if (executingDayOfWeek != 7) {
                                BukkitRunnable r = new BukkitRunnable() {
                                    ...
                                    public void run() {
                                        try {
                                            openConnection();
                                            Statement statement = connection.createStatement();
                                            String SQLString = "DELETE FROM FarmsTableTest WHERE type='1' AND PlayerUUID='" + player.getUniqueId() + "' AND name='" + strings[1] + "';";
                                            statement.executeUpdate(SQLString);
                                            connection.close();
                                        } catch (ClassNotFoundException | SQLException e) {
                                            e.printStackTrace();
                                ...
                                r.runTaskAsynchronously(JavaPlugin.getPlugin(SubTaxes.class));
                                player.sendMessage("Ваша ферма " + strings[1] + " успешно удалена из списка на удаление!");
                                return true;
                            } else player.sendMessage("Данная команда работает во все дни, кроме воскресенья!");
                        }
                    }
        ...
        if (command.getName().equalsIgnoreCase("farms")) {
            if (strings.length == 1) {
                if (strings[0].equals("list")) {
                    ...
                    r.runTaskAsynchronously(JavaPlugin.getPlugin(SubTaxes.class));
                    player.sendMessage("Список ваших ферм на рассмотрении:");
                    BukkitRunnable r2 = new BukkitRunnable() {
                        ...
                        public void run() {
                            try {
                                ...
                                openConnection();
                                Statement statement = connection.createStatement();
                                ResultSet result = statement.executeQuery("SELECT * FROM FarmsTableTest;");
                                while (result.next()) {
                                    UUID ownerUUID = UUID.fromString(result.getString("playerUUID"));
                                    String name = result.getString("name");
                                    if (ownerUUID.equals(targeterUUID)) {
                                        player.sendMessage("У вас на рассмотрении имеется ферма с названием: " + name + ".");
                                    }
                                    ;
                                }
                                connection.close();
                    ...
                    r2.runTaskAsynchronously(JavaPlugin.getPlugin(SubTaxes.class));
                    return false;
                }
            }
        }
